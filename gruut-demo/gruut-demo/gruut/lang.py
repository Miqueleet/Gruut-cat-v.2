...
def phoneme_is_midlow_vowel(phoneme: str) -> bool:
    return phoneme in MIDLOW_VOWELS

def phoneme_is_midlow_stressed_vowel(phoneme: str) -> bool:
    return phoneme_is_midlow_vowel(phoneme) and phoneme_is_stressed_vowel(phoneme)

def phoneme_is_midlow_unstressed_vowel(phoneme: str) -> bool:
    return phoneme_is_midlow_vowel(phoneme) and phoneme_is_unstressed_vowel(phoneme)

def phoneme_is_voiced(phoneme: str) -> bool:
    return phoneme in VOICED_PHONEMES

def phoneme_is_unvoiced(phoneme: str) -> bool:
    return phoneme in UNVOICED_PHONEMES

def phoneme_is_aproximated(phoneme: str) -> bool:
    return phoneme in APROXIMATED_PHONEMES

def fusion_if_needed(node_1: WordNode, node_2: WordNode, lang: str):

    if lang in ["ca", "ca-ce","ca-ba","ca-no","ca-va"]:
        if len(node_1.phonemes) == 0 or len(node_2.phonemes) == 0:
            ...
            if phoneme_is_high_stressed_vowel(last_phoneme_word_1) and phoneme_is_high_unstressed_vowel(first_phoneme_word_2) \
                and last_phoneme_word_1.replace("'", "") == first_phoneme_word_2:
                # Case ['i] + [i] = ['i] or ['u] + [u] = ['u]
                node_1.phonemes.pop()
            ...
            # Case 4: [e] [a] and [o] fusion for balearic (o), and western dialects (aoe) 
            elif phoneme_is_midlow_unstressed_vowel(last_phoneme_word_1) and phoneme_is_midlow_unstressed_vowel(first_phoneme_word_2) \
                and last_phoneme_word_1 == first_phoneme_word_2:
                node_1.phonemes.pop()
                _LOGGER.debug(f"FUSION CASE 1 {node_1.text} {node_2.text}: {node_1.phonemes} {node_2.phonemes}")  

            # Case 5: ['e + e] ['a + a] and ['o + o] fusion for balearic (o), and western dialects (aoe) 
            elif phoneme_is_midlow_stressed_vowel(last_phoneme_word_1) and phoneme_is_midlow_unstressed_vowel(first_phoneme_word_2) \
                and last_phoneme_word_1.replace("'", "") == first_phoneme_word_2:
                node_1.phonemes.pop()
                _LOGGER.debug(f"FUSION CASE 1 {node_1.text} {node_2.text}: {node_1.phonemes} {node_2.phonemes}")      
    else:
...
def elision_if_needed(node_1: WordNode, node_2: WordNode, lang: str):
    ...
    if lang in ["ca", "ca-ce","ca-ba","ca-no","ca-va"]:
        ...
        if len(node_1.phonemes) == 0 or len(node_2.phonemes) == 0:
            ...
            # Case 3: western dialects [a elision] 
            elif last_phoneme_word_1 == "o" and (first_phoneme_word_2 == "e" or first_phoneme_word_2 == "a"):
                node_2.phonemes.pop(0)
                _LOGGER.debug(f"ELISION CASE 2 {node_1.text} {node_2.text}: {node_1.phonemes} {node_2.phonemes}")  

            # Case 3: western dialects [a elision] and balear (o + neutral)
            elif last_phoneme_word_1 == "o" and (first_phoneme_word_2 == "e" or first_phoneme_word_2 == "a" or first_phoneme_word_2 == "ə"):
                node_2.phonemes.pop(0)
                _LOGGER.debug(f"ELISION CASE 2 {node_1.text} {node_2.text}: {node_1.phonemes} {node_2.phonemes}")      
    else:
        pass    
...
def diphthong_if_needed(node_1: WordNode, node_2: WordNode, lang: str):
    ...
    if lang in ["ca", "ca-ce","ca-ba","ca-no","ca-va"]:
        ...
        pass   
def voicing_if_needed(node_1, node_2, lang):
    if lang in ["ca", "ca-ce", "ca-ba", "ca-no", "ca-va"]:
        if len(node_1.phonemes) == 0 or len(node_2.phonemes) == 0:
            return
        else:
            ...
            if phoneme_is_unvoiced(last_phoneme_word_1) and phoneme_is_voiced(first_phoneme_word_2):
                if last_phoneme_word_1 in VOICING:
                    node_1.phonemes[-1] = VOICING[last_phoneme_word_1]
                    _LOGGER.debug(f"DIPTHONG CASE 1 {node_1.text} {node_2.text}: {node_1.phonemes} {node_2.phonemes}")
                    
    else:
        pass   
def lenition_if_needed(node_1, node_2, lang):
    if lang in ["ca", "ca-ce", "ca-ba", "ca-no", "ca-va"]:
        if len(node_1.phonemes) == 0 or len(node_2.phonemes) == 0:
            return
        else:
            ...
            if phoneme_is_vowel(last_phoneme_word_1) and phoneme_is_aproximated(first_phoneme_word_2):
                if first_phoneme_word_2 in LENITION:
                    node_2.phonemes[0] = LENITION[first_phoneme_word_2]
                    _LOGGER.debug(f"DIPTHONG CASE 1 {node_1.text} {node_2.text}: {node_1.phonemes} {node_2.phonemes}")
...
def ca_post_process_sentence(
    ...
    for node in nodes:
        ...
        if isinstance(node, WordNode):
            ...
            count = phonemes_text.count("'")
            if count >= 2:
                phonemes_text_new = phonemes_text.replace("'", "", 1)
                if lang in ["ca","ca-ce"] and node.text[-4:]!="ment":
                    for key, value in vowel_reduction_eastern.items() :
                        phonemes_text_new = phonemes_text_new.replace(key, value)
                    node.phonemes = phonemes_text_new.split(" ")
                elif lang in ["ca-no","ca-va"] and node.text[-4:]!="ment":
                    for key, value in vowel_reduction_western.items() :
                        phonemes_text_new = phonemes_text_new.replace(key, value)
                    node.phonemes = phonemes_text_new.split(" ")
                elif lang in ["ca-ba"]:
                    for key, value in vowel_reduction_balearic.items() :
                        phonemes_text_new = phonemes_text_new.replace(key, value)
                    node.phonemes = phonemes_text_new.split(" ")    
    ...
    for (node_1, node_2) in contiguous_word_nodes:
        ...
        voicing_if_needed(node_1,node_2,lang)
        lenition_if_needed(node_1,node_2, lang)
...
E_OBERTA = ["enc","el","ert","erd","erg","erm","ern","erc","erb","erp","eig", \
                    "enca","ela","erta","erda","erga","erma","erna","erca","erba","erpa","eja", \
                    "encs","els","erts","erds","ergs","erms","erns","ercs","erbs","erps","eigs", \
                    "enques","eles","ertes","erdes","ergues","ermes","ernes","erques","erbes","erpes","ejes",]

E_TANCADA = ["er","ell","ent","eix","et","eta","ets","etes","ed"] 

O_OBERTA = ["ot","ol","oc","op","org","oig", \
                    "ota","ola","oca","opa","oja", \
                    "ots","ols","ocs","ops","orgs","ojos", \
                    "otes","oles","oques","opes","ojes"]

O_TANCADA = ["ora","ores","osa","oses","oni","onis","onin","ona","ones","osa","oses","orra","orres","onya","onyes"]
...
class Transcripcio:
    ...
    def stress_tonic(self) -> str:
        ...
        def ends_with_any(s: str, suffixes: list) -> bool:
            return any(s.endswith(suffix) for suffix in suffixes)

        
        
        E_OBERTA = ["el","ert","erd","erg","erm","ern","erc","erb","erp","eig", \
                    "ela","erta","erda","erga","erma","erna","erca","erba","erpa","eja",\
                    "els","erts","erds","ergs","erms","erns","ercs","erbs","erps","eigs", "et","eta","ets","etes", \
                    "eles","ertes","erdes","ergues","ermes","ernes","erques","erbes","erpes","ejes","em","enc","enca","encs","enques"]

        E_TANCADA = ["er","era","ers","eres","ell","ella","ells","elles","ent","ed","eça","eces","ea","ees",\
                     "ens","entra","essa","enta","entes","ents"] 

        O_OBERTA = ["ot","ol","oc","op","org","oig", \
                    "ota","ola","oca","opa","oja", \
                    "ots","ols","ocs","ops","orgs","ojos", \
                    "otes","oles","oques","opes","ojes","olo","olen"]

        O_TANCADA = ["ora","ores","osa","oses","oni","onis","onin","ono","ona","ones","osa","oses","osos","orra","orres","onya","onyes","oma","omes","onze","oixa","oa","oes"]
...
PHONEME_VOWELS = ["'a", "'ɛ", "'ɔ", "'e", "'i", "'o", "'u", "ə", "i", "u" , "a" , "o" , "e" ]
PHONEME_STRESSED_VOWELS = ["'a", "'ɛ", "'ɔ", "'e", "'i", "'o", "'u"]
PHONEME_HIGH_VOWELS = ["i" , "u" , "'i", "'u" ]
PHONEME_NEUTRAL_VOWELS = ["ə"]
MIDLOW_VOWELS = [ "e" , "o" , "a" , "'e" , "'a" , "'o" ]
VOICED_PHONEMES=["ə","a","e","ɛ","i","o","ɔ","u","'ə","'a","'e","'ɛ","'i","'o","'ɔ","'u","b","d","ɡ","ʎ","m","n","ɲ","ŋ","l","r","ɾ","z","ʒ","dʒ","β","ð","ɣ","w","j","v"]
UNVOICED_PHONEMES=["f", "s", "ʃ", "ts", "tʃ"]
APROXIMATED_PHONEMES=["b", "d", "g"]
DEVOICING={
    "b": "p",
    "d": "t",
    "ɡ": "k",
